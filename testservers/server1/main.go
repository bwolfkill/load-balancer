...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintf(w, "You are now connected to Server 1.")
	})
	http.HandleFunc("/slow", func(w http.ResponseWriter, r *http.Request) {
		time.Sleep(25 * time.Second)
		_, _ = fmt.Fprintf(w, "Slow response from Server 1.")
	})
	http.HandleFunc("/sethealthy", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("healthy") == "false" {
			healthy.Store(false)
			_, _ = fmt.Fprintf(w, "Server 1 set to unhealthy")
		} else {
			healthy.Store(true)
			_, _ = fmt.Fprintf(w, "Server 1 set to healthy")
		}
	})
